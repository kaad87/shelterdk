...
export async function GearCard({
  ...
  if (!product) {
    // Fallback: empty span. Visible-only-in-devtools trace for debug.
...
export function GearCardView({
  ...
  if (variant === "editorial")
    return <EditorialVariant product={product} className={className} />;
  if (variant === "product")
    return <ProductVariant product={product} className={className} />;
...
function RetailerLabel({
  ...
  retailer: AffiliateProduct["retailer"];
}) {
  const labels = {
    outmore: "Outmore.dk",
...
function PillVariant({
  ...
  return (
    ...
    >
      ...
      {outOfStock && (
        <span className="text-[11px] text-primary/50">(udsolgt)</span>
      )}
    </a>
  );
}
