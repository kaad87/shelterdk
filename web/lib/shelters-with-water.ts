/**
 * Henter shelters hvor der er vand (DB-kolonnen water = true, eller geofa_raw.vandhane).
 * Bruges til SEO-landingssiden /shelter-med-vand.
 */
...
export async function getSheltersWithWater(limit: number = 200): Promise<Shelter[]> {
  ...
  if (error) {
    console.error("Supabase error (shelters with water):", error);
    return [];
  }
