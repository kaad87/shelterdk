...
import { createBooking, getBookableShelterByOwnerToken } from "@/lib/booking-db";
...
export async function POST(
  ...
  if (!guestName) return NextResponse.json({ error: "Navn er påkrævet" }, { status: 400 });
  if (!isIsoDate(checkIn) || !isIsoDate(checkOut) || checkOut <= checkIn) {
    return NextResponse.json({ error: "Ugyldige datoer" }, { status: 400 });
  }
  if (!Number.isInteger(guestCount) || guestCount < 1 || guestCount > shelter.max_persons) {
    return NextResponse.json(
      { error: `Antal personer skal være mellem 1 og ${shelter.max_persons}` },
      { status: 400 }
    );
  }
  ...
  const { data: overlapRows, error: overlapError } = await admin
    ...
    .select("id")
    .eq("bookable_shelter_id", shelter.id)
  ...
  const guestEmail =
    guestEmailRaw ||
    `manual+${shelter.id.slice(0, 8)}-${Date.now()}@placeholder.shelterdk.local`;
